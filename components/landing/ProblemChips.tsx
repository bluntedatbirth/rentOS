...
export function ProblemChips({ t, chips }: ProblemChipsProps) {
  return (
    <section className="px-6 pt-6 pb-14 bg-warm-50 dark:bg-charcoal-900">
      <AnimatedSection className="max-w-[900px] mx-auto text-center">
        ...
        <div className="flex flex-wrap gap-2.5 justify-center">
          {chips.map((chip, i) => (
            <span
              key={i}
              className="bg-saffron-500/10 dark:bg-charcoal-800 border border-saffron-500/25 dark:border-white/10 rounded-sm px-4 py-2 text-sm font-medium text-charcoal-800 dark:text-warm-200"
            >
              {chip}
            </span>
          ))}
