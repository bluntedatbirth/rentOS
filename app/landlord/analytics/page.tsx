  ...
  return (
    <div className="rounded-lg bg-white p-4 shadow-sm">
      ...
      <div className="mt-1 flex items-end gap-2">
        ...
        {badge && (
          <span
            className={`mb-0.5 rounded-full px-2 py-0.5 text-xs font-semibold ${badgeColor ?? 'bg-gray-100 text-gray-600'}`}
          >
            {badge}
          </span>
        )}
...
function BarChart({ data }: { data: { month: string; revenue: number; expected: number }[] }) {
  ...
  return (
    <div className="rounded-lg bg-white p-4 shadow-sm">
      <h3 className="mb-4 text-sm font-semibold text-gray-700">{t('analytics.revenue_trend')}</h3>
      {/* Legend */}
          ...
          {data.map((d) => {
            ...
            return (
              ...
              >
                ...
                {/* Month label */}
                <span className="text-[10px] text-gray-400">{shortMonth}</span>
                {/* Tooltip */}
                <div className="pointer-events-none absolute bottom-full mb-2 hidden w-28 rounded-lg bg-gray-900 p-2 text-center text-xs text-white shadow-lg group-hover:block">
...
function PropertyTable({ data }: { data: AnalyticsData['property_performance'] }) {
  ...
  const thCls =
    'min-h-[44px] cursor-pointer select-none px-3 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hover:text-gray-700';
          ...
          <tbody className="divide-y divide-gray-50">
            {sorted.map((row) => (
              <tr key={row.property_id} className="hover:bg-gray-50">
                <td className="px-3 py-3 text-sm font-medium text-gray-900">{row.property_name}</td>
                <td className="px-3 py-3 text-sm text-gray-700">
                  {row.monthly_rent > 0 ? fmt(row.monthly_rent) : '—'}
                </td>
                <td className="px-3 py-3 text-sm text-gray-700">{fmt(row.total_collected)}</td>
                <td
                  className={`px-3 py-3 text-sm font-medium ${
                    row.total_overdue > 0 ? 'text-red-600' : 'text-gray-500'
                  }`}
                >
                ...
                <td className="px-3 py-3">
                  <span
                    className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                      row.is_occupied ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                    }`}
                  >
          ...
          sorted.map((row) => (
            <div key={row.property_id} className="px-4 py-4">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
                <div>
                  <p className="text-xs text-gray-400">{t('analytics.monthly_rent')}</p>
                  <p className="font-medium text-gray-700">
                    {row.monthly_rent > 0 ? fmt(row.monthly_rent) : '—'}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">{t('analytics.collected')}</p>
                  <p className="font-medium text-gray-700">{fmt(row.total_collected)}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-400">{t('analytics.overdue')}</p>
                  <p
...
export default function AnalyticsPage() {
  ...
  // Pro gate for client-side (before even hitting the API, if tier is known)
  if (
    !loading &&
    !blocked &&
    profile &&
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-3xl">
        <h2 className="mb-6 text-xl font-bold text-gray-900">{t('analytics.title')}</h2>
    ...
    return (
      <div className="mx-auto max-w-3xl rounded-lg bg-white p-8 text-center shadow-sm">
        ...
        <button
          type="button"
          onClick={load}
          className="mt-4 min-h-[44px] rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"
        >
          {t('common.retry')}
        </button>
      </div>
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard
          ...
          valueColor={data.revenue.total_outstanding > 0 ? 'text-red-600' : 'text-gray-900'}
        />
        <StatCard
        ...
        <StatCard
          ...
          valueColor={data.payments.late_payment_rate > 20 ? 'text-red-600' : 'text-gray-900'}
        />
      </div>
