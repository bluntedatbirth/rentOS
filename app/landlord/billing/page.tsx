...
export default function BillingDashboardPage() {
  ...
  const planCardClass = isPro
    ? 'rounded-lg border border-saffron-300 bg-saffron-50 p-6 shadow-sm'
    : 'rounded-lg border border-warm-200 bg-white p-6 shadow-sm';

  return (
    <div className="mx-auto max-w-3xl space-y-6 px-4 pb-12">
      <h1 className="text-xl font-bold text-charcoal-900">{t('billing.dashboard_title')}</h1>

      {loading ? (
        // Skeleton: three card placeholders so the page never renders empty
        <>
          <div className="h-32 animate-pulse rounded-lg border border-warm-200 bg-warm-50" />
          <div className="h-32 animate-pulse rounded-lg border border-warm-200 bg-warm-50" />
          <div className="h-32 animate-pulse rounded-lg border border-warm-200 bg-warm-50" />
        </>
      ) : (
        <>
          ...
          <section className={planCardClass}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              {isFoundingMember && (
                <span className="inline-flex shrink-0 items-center self-start rounded-full border border-saffron-300 bg-saffron-100 px-3 py-1 text-xs font-semibold text-saffron-800">
          ...
          <section className="rounded-lg border border-warm-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              {isSimulated && (
                <span className="inline-flex shrink-0 items-center self-start rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-800">
          ...
          <section className="rounded-lg border border-warm-200 bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-2">
              {PRO_BULLET_KEYS.map((key) => (
                <li key={key} className="flex items-start gap-2 text-sm text-charcoal-700">
                  <svg
