...
export function PenaltiesClient({ initialPenalties, initialContracts }: PenaltiesClientProps) {
  ...
  const [contractMap] = useState<Record<string, Contract>>(() => {
  ...
  const reloadPenalties = useCallback(async () => {
    ...
    const sorted = ((data ?? []) as Penalty[]).sort((a, b) => {
          ...
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            {modalStep === 3 && calcResult && (
              <div>
                ...
                <div className="mb-4 rounded-lg bg-saffron-50 p-4">
                  <p className="mb-1 text-xs font-medium text-saffron-700">
            ...
            return (
              ...
              >
                ...
                {p.tenant_appeal_note && (
                  <div className="mt-3 rounded-lg bg-warm-100 p-3">
                    <p className="text-xs font-medium text-charcoal-600">
                      {t('tenant.your_appeal')}
                    </p>
                    <p className="mt-1 text-sm text-charcoal-800">{p.tenant_appeal_note}</p>
                  </div>
                )}
