    ...
    <div className="min-h-screen bg-gray-50">
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <Link href="/landlord/dashboard" className="text-lg font-bold text-gray-900">
            {t('app.title')}
          </Link>
        ...
        <nav className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-4 pb-2">
          {[
            ...
            { href: '/landlord/properties', label: t('nav.properties') },
            { href: '/landlord/contracts/upload', label: t('nav.upload_contract') },
          ].map((link) => {
            const isActive =
              pathname === link.href ||
              (link.href !== '/landlord/dashboard' && pathname.startsWith(link.href));
            return (
              <Link
                key={link.href}
                href={link.href}
