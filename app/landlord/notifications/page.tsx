...
export default function NotificationSettingsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    const load = async () => {
      // Load from profile metadata or a settings table
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {/* Custom Rules link */}
      <Link
        href="/landlord/notifications/rules"
        className="mb-6 flex items-center justify-between rounded-xl bg-white p-4 shadow-sm hover:bg-gray-50"
      >
        <div>
          <p className="font-medium text-gray-900">{t('notification_rules.title')}</p>
          <p className="mt-0.5 text-sm text-gray-500">
            {t('notification_rules.settings_link_desc')}
          </p>
        </div>
        <svg
